...
def serialize_data(serializer, target, num_steps, padding = -1):
    ret_input = []
    ret_output = []
    eos_id = serializer.char2id(serializer.EOS)
    for data in target:
        tmp = [serializer.char2id(c) for c in data]
        tmp_input  = [-1] * (num_steps - len(tmp)) + tmp
        tmp_output = tmp + [eos_id] + [-1] * (num_steps - len(tmp))
        assert(len(tmp_input) + 1 == len(tmp_output))

        ret_input.append(tmp_input)
        ret_output.append(tmp_output)

    return np.array(ret_input, dtype = np.int32),\
        np.array(ret_output, dtype = np.int32)
